...
Ниже рабочая схема для текущего проекта:
- домен: `botmaks.ru`
...
## 12. Защита страницы поиска через nginx

`/search.html` защищается через `auth_request`.

Это значит:
- если сессии нет, nginx не отдаст страницу поиска;
- вместо нее пользователь будет возвращен на `/` со страницей входа;
- даже прямое открытие `/search.html` без cookie не покажет защищенный интерфейс.
